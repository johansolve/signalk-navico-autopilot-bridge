...
class AccessRequest {
  ...
  request (method, path, body, cb) {
    // Single-fire the callback: a late 'error' after 'end' would otherwise trigger
    // an extra submit/poll chain.
    let done = false
    const finish = (err, code, parsed) => { if (done) { return } done = true; cb(err, code, parsed) }
    try {
      ...
      // synchronous throw on bad host/port -- surface as a normal callback error.
      finish(e)
    }
  }
