...
class ACEmulator {
  constructor (app, options) {
    ...
    this.legSince = 0           // when the current leg's bearing was first seen (held-duration)
  ...
  sampleTrackBrg () {
    ...
    if (this.legBrg === null) { this.legBrg = b; this.legWp = wp; this.legSince = now; return }
    const jump = this.angDiffDeg(b, this.legBrg)
    if (jump > 1) {                              // bearing jumped -> advance, or a leg re-origin
      // Same destination across the jump = the MFD's Restart re-origining the leg on the boat.
      // A different -- or unknown -- destination is called an advance, so the guarded path
      // stays the default whenever the classification is not solid.
      const moved = this.posDistM(wp, this.legWp)
      this.lastTurnKind = (moved !== null && moved <= RESTART_SAME_WP_TOL_M) ? 'restart' : 'advance'
      this.lastTurnDeg = jump
      this.lastTurnOldHeldMs = now - this.legSince
      this.lastTurnAt = now
      this.legBrg = b
      // legWp moves ONLY in lockstep with legBrg, so the comparison above is always "destination
      // now vs destination when this leg's bearing was set". Re-baselining it on every sample
      // instead would let an advance whose bearing settles across two frames -- the first step
      // under 1 deg, so no jump is recorded but the destination has already moved on -- compare
      // the new waypoint against itself and read as a re-origin, handing a real advance the
      // restart path's relaxed gates. An advance whose bearing never crosses 1 deg now leaves
      // legWp stale instead, which biases the NEXT jump toward 'advance': the safe direction.
      this.legWp = wp
      this.legSince = now
    }
    ...
    // 36 h of recorded model data and dropped without ever being deployed. What that measurement
    // established, so nobody has to find it a third time:
    //
    ...
    //   * Any size limit on this path is decorative. maybeAutoAdvance() samples first, so a bearing
    //     more than 1 deg from legBrg has already been recorded as a turn and never reaches a
    //     collinear branch; one within 1 deg leaves legBrg untouched and the branch can only ever
    //     compute a sub-degree answer. The number it reports is not a measurement of the turn.
    //   * The collinear case is rarer than it looks. Of 50 leg changes flown in route, 5 measured
    //     under 1 deg at the change sample but only 3 were genuinely sub-degree once the bearing
    //     settled, and 1 survived the settled-leg gate. One qualifying advance in 3.7 h of routing.
    //
    // The real population is elsewhere. Of 16 recorded 'turn not sizeable' refusals, 9 had no
    // destination change within 15 s at all (a pending that is not an advance), and 3 failed the
    // 5 s settled gate because the bearing had jumped again 1-3 s earlier. Note also that
    // lastTurnDeg is only cleared when a pending consumes it, so 'deg === null' means "nobody has
    // measured anything since the last pending", not "the bearing recorded nothing" -- a value
    // 422 s old was observed still latched. Start there, not here.
  }
  ...
  maybeAutoAdvance () {
    ...
    if (kind === 'restart' && this.autoConfirmRestart) {
      ...
      if ((now - this.lastTurnAt) > RESTART_PENDING_MS || this.lastTurnOldHeldMs < 5000) {
        this.state.lastMappedEvent = 'Track restart, re-origin not trustworthy -> manual confirm needed'
        this.debug('restart: no settled recent re-origin to act on -- manual confirm')
        return
      }
      if (deg > RESTART_MAX_DEG) {
        this.state.lastMappedEvent = `Track restart ${Math.ceil(deg)}deg > ${RESTART_MAX_DEG}deg -> manual confirm needed`
        this.debug(`restart re-origin ${deg.toFixed(1)}deg exceeds ${RESTART_MAX_DEG}deg -- manual confirm`)
        return
      }
      this.state.lastMappedEvent = `Track restart ${deg.toFixed(0)}deg (same waypoint) -> engage track`
    ...
    if (this.autoAdvanceMaxDeg <= 0) {
      this.state.lastMappedEvent = 'Advance pending -> manual confirm needed (auto-advance off)'
      return
    }
    // Require a turn that (a) just happened, coincident with this pending (<=3 s), and (b)
    // left a leg that had been established long enough to trust (>=5 s -- a short-lived leg,
    // or a stale cross-route value, must never size the turn).
    if (deg === null || (now - this.lastTurnAt) > 3000 || this.lastTurnOldHeldMs < 5000) {
      this.state.lastMappedEvent = 'Advance pending, turn not sizeable -> manual confirm needed'
      this.debug('auto-advance: no trustworthy recent leg change to size the turn -- manual confirm')
      return
    }
    if (deg <= this.autoAdvanceMaxDeg) {
      this.state.lastMappedEvent = `Auto-advance ${deg.toFixed(0)}deg <= ${this.autoAdvanceMaxDeg}deg -> engage track`
      ...
      this.state.lastMappedEvent = `Advance ${Math.ceil(deg)}deg > ${this.autoAdvanceMaxDeg}deg -> manual confirm needed`
