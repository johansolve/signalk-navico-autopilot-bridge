...
// on "auto".
//
// 65340/65302 were dropped 2026-07-22 because a dockside test found them costing neither
// the binding nor the mode label, and most of the byte values are htool guesses. Restored
// 2026-08-06 on the one ground that survives scrutiny: a real AC42 emits BOTH -- 65340 and
// 65302 alike appear from src 13 in `ac42-comm` -- and the point of the emulator is to look
// like one. They cost two frames a second.
//
// They are NOT a fix for the two users stuck at a first commissioning. Issue #1 was filed
// four days BEFORE the drop, against 0.7.0-beta, which sent both: that plotter failed with
// the frames on the wire and again without them. Do not read the restore as a diagnosis.
//
// Restored unchanged rather than corrected to ground truth. The capture has AC42 standby at
// `41 9f 0a 2b ...` where the htool row here says `0a 6b`, and the route row of 65302 is
// htool's own explicit guess -- but these exact bytes ran on the reference rig from 0.1.0
// to 0.7.0 without trouble, and changing values and re-enabling transmission in one step
// would leave nothing to attribute a new symptom to. See PROTOCOL-REFERENCE §3.3.
const MODE_65340 = {
...
// How long a commanded mode may outrank the V2 poll before we conclude the pilot never took it.
// Same grace the display reconciliation in pollState gives, for the same reason. Normally the
// poll confirms the mode long before this and closes the window itself -- see keyMode.
const MODE_ADOPT_MS = 5000
...
// A leg re-origin collapses cross-track error onto zero, which is what the MFD's Restart does by
// definition. Measured over 2026-08-04: at these thresholds the day held exactly three collapses
// -- 21.7, 28.9 and 35.1 m, each to under half a metre -- and all three were genuine, each with a
// Track-pending in front of it. Relaxing the entry to 5 m adds three more: two that look like
// restarts in their own right, and one that coincides with a destination change, i.e. an advance,
// which the branch above claims first anyway. Held at 10 m because nothing needs the extra two:
// their bearings moved 2-6 deg, so the existing re-origin branch already sizes them.
const RESTART_XTE_FROM_M = 10
const RESTART_XTE_TO_M = 2
...
class ACEmulator {
  constructor (app, options) {
    ...
    this.commandedAt = 0            // when a mode-bearing key was last pressed
    // When commandedMode was last set to a NEW mode we asked the pilot for. Distinct from
    // commandedAt, which two Nav paths bump without touching commandedMode, and which the
    // rollback and reconciliation paths leave alone while they rewrite it -- so it cannot
    // answer "how old is this mode". keyMode() needs that answer and nothing else does.
    this.commandedModeAt = 0
    // Whether a poll has since returned commandedMode, i.e. the pilot is known to have taken it.
    // Starts true: the initial 'standby' is an assumption, not something we asked for, and must
    // not give keyMode a window before the first key is ever pressed.
    this.commandedModeSeen = true
    ...
    this.legXte = null              // last cross-track error on THIS leg; a collapse is a re-origin
    this.lastTurnXteFromM = null    // what it collapsed from, for the decision record
  ...
  // The boat's own fix. Only the re-origin path uses it, where it IS the new leg's origin rather
  // than a stand-in for one.
  vesselPos (maxAgeMs) {
    try {
      const n = this.app.getSelfPath('navigation.position')
      const p = n && n.value
      if (!p || !Number.isFinite(p.latitude) || !Number.isFinite(p.longitude)) { return null }
      if (p.latitude === 0 && p.longitude === 0) { return null }
      const t = Date.parse(n.timestamp)
      return (Number.isFinite(t) && (Date.now() - t) <= maxAgeMs) ? p : null
    } catch (e) { return null }
  }
  ...
  // The mode a key must be interpreted against. state.skApState is a poll of the V2 API and is
  // only refreshed every 2 s, so for up to that long after a mode key it still names the mode we
  // just LEFT -- and a key read against it is read against the wrong mode. Seen on the water
  // 2026-08-04: a '-' nudge pressed 1.23 s after Auto came out of wind was wind-inverted anyway
  // and turned the boat ten degrees the wrong way, twice, until the poll caught up.
  //
  // commandedMode is set synchronously by the mode key, so until the pilot is known to have
  // adopted it, it is the better answer.
  //
  // "Known to have adopted it" and not "newer than the last poll", because the V2 state is NOT
  // optimistic: signalk-autopilot only moves it when the EV-200 broadcasts its own 65379, so
  // there is a gap between our PUT and the API agreeing. A poll issued inside that gap returns
  // the old mode perfectly legitimately, and a timestamp test would read it as the pilot having
  // spoken and hand the stale mode back -- the original bug, reachable through a narrower door.
  // So the poll only closes the window by returning the mode we asked for. commandedModeSeen
  // latches that, and once latched the poll is authoritative again: a pilot that later leaves
  // the mode on its own (the p70s beside the wheel does exactly that) is believed immediately.
  //
  // MODE_ADOPT_MS bounds the wait for a pilot that never adopts it at all, and matches the grace
  // the display reconciliation in pollState already gives the same command.
  //
  // A commanded 'standby' is not trusted, and that is not tidiness. Standby is a mode key like any
  // other, so it sets commandedMode -- but the nudge branch is gated on engaged(), which reads the
  // poll. Between a Standby press and the next poll, engaged() still says wind and would let a
  // nudge through while this said 'standby': no inversion, and the boat turns the wrong way for a
  // pilot that is physically still in wind vane. Falling back on the poll keeps the old behaviour
  // in that window, which was right. The interesting case is leaving standby, and that is unaffected.
  //
  // Deliberately NOT used by engaged(): that one gates whether we may drive the pilot at all, and
  // a mode we merely asked for is no evidence the pilot left standby. Optimism may pick the sign
  // of a nudge; it may not authorise one.
  //
  // Known gap, left alone on purpose: a mode change made on the p70s at the wheel, or the moment
  // the pilot reaches Track-engaged, is only learned from the poll, so a nudge in the two seconds
  // after one is still signed against the previous mode. Closing it means trusting the sniffed
  // 65379 over the V2 API, which would tie the sign of every nudge to Raymarine-specific decoding
  // in a bridge that is otherwise provider-agnostic. Neither case has been seen to bite; the one
  // that has -- a mode key on the MFD -- is what this handles.
  // Record a mode we have just asked the pilot for. The fields have to move together -- a
  // commandedMode set without clearing the seen latch is a window that never opens, and one set
  // without the timestamp is a window that never closes -- so nothing sets them separately.
  //
  // `adopted` closes the keyMode window immediately, for a mode that is not a bet on what the
  // pilot is about to do: either it is already proven there (the sniffed 0x0181), or the mode is
  // route, where the pilot reaches Track through a confirm dialog on its own schedule and may sit
  // in wind vane meanwhile -- claiming route there would drop the wind inversion out from under a
  // nudge. Only a mode key, which the EV adopts within about a second, is worth being optimistic
  // about.
  commandMode (mode, adopted) {
    this.commandedMode = mode
    this.commandedAt = Date.now()
    this.commandedModeAt = this.commandedAt
    this.commandedModeSeen = adopted === true
  }

  // Undo the optimism after the pilot refused a mode, restoring what it is actually in. The
  // window has to close with it: the command has been withdrawn, so it must stop outranking the
  // poll -- otherwise a pilot that reaches the mode by some other route afterwards is answered
  // with the rolled-back one, and a nudge is signed against a mode nobody is in.
  //
  // Timestamps are deliberately untouched. commandedAt gates the display reconciliation and the
  // Nav engage proof, both of which measure from the KEY PRESS; restamping it here would grant a
  // refused command a fresh five seconds of both.
  withdrawMode (mode) {
    this.commandedMode = mode
    this.commandedModeSeen = true
  }
  ...
  keyMode () {
    const m = this.normalizeMode(this.commandedMode)
    if (m !== null && m !== 'standby' && !this.commandedModeSeen &&
        (Date.now() - this.commandedModeAt) < MODE_ADOPT_MS) {
      return m
    }
    return this.normalizeMode(this.state.skApState)
  }
  ...
  pollState () {
    ...
    this.sk.getState((err, st) => {
      if (!err && st && this.normalizeMode(st) !== null) {
        ...
        // The pilot has been seen in the mode we asked for, so keyMode can stop preferring the
        // commanded one. Latched rather than compared per call: after this, a mode the pilot
        // leaves on its own must be believed at once.
        if (this.normalizeMode(st) === this.normalizeMode(this.commandedMode)) {
          this.commandedModeSeen = true
        }
        this.pollWarned = false
  ...
  sampleTrackBrg () {
    ...
    // Read once, up here, and re-baseline it wherever the leg itself is re-baselined. Held on the
    // branch that uses it instead, it survived a waypoint advance -- that branch returns before
    // any update -- and the next sample then compared the OLD leg's cross-track error against the
    // NEW leg's, which starts near zero by construction. That is a collapse on paper every time.
    // Null when the path goes quiet, so a value from before the silence can never be compared
    // against one after it.
    const xte = this.selfPathNumFresh('navigation.courseGreatCircle.crossTrackError', LEG_MAX_AGE_MS)
    const now = Date.now()
    if (b === null) {
      this.legBrg = null; this.legWp = null; this.legSince = 0
      this.legXte = null
      this.lastTurnDeg = null; this.lastTurnKind = null
      return
    }
    if (this.legBrg === null) {
      this.legBrg = b; this.legWp = wp; this.legSince = now; this.legXte = xte
      return
    }
    // Previous SAMPLE, not the value at the start of the leg. A re-origin collapses cross-track
    // error between one sample and the next; sailing back onto the line does the same thing over
    // a minute, and against a leg-start baseline the two are indistinguishable. Stored before any
    // branch runs so every path leaves it pointing at this sample.
    const wasXte = this.legXte
    this.legXte = xte
    ...
    // A re-origin the bearing was too small to show. The MFD's Restart re-lays the leg from the
    // boat, so cross-track error collapses onto zero -- that is what it does, and it is visible
    // whatever the bearing did. How far it moves the bearing depends entirely on where along the
    // leg it was pressed, so the 1 deg gate above is not a threshold that can be tuned: measured
    // 0.3 deg on a 4.9 km leg (2026-08-03) and 0.940 deg on 2026-08-04, both unmistakable in the
    // cross-track trace (29 m to zero in the second case) and both a control-head press.
    //
    // The size does NOT come from the bearing here. It comes from the boat's own position, which
    // for a re-origin is not an approximation but the definition: the new leg runs from the boat
    // to the destination it was already bound for. Verified against the 2026-08-04 restart --
    // geometry 152.2 deg, plotter published 152.2 deg. Unlike previousPoint it needs no settling
    // time, so it is available at the instant the pending arrives.
    //
    // Taking the maximum of the two matters most in the case this exists to make safe: a Restart
    // pressed with the waypoint abaft the beam asks for a near-reciprocal turn, and if the bearing
    // has not caught up, `jump` reads zero. The geometry reads the real angle, RESTART_MAX_DEG
    // refuses it, and the skipper is asked -- which is the whole point of that cap.
    if (jump <= 1 && moved !== null && moved <= RESTART_SAME_WP_TOL_M && wp &&
        xte !== null && wasXte !== null &&
        Math.abs(wasXte) > RESTART_XTE_FROM_M && Math.abs(xte) < RESTART_XTE_TO_M) {
      const vp = this.vesselPos(LEG_MAX_AGE_MS)
      const geoBrg = vp ? this.posBrgRad(vp, wp) : null
      // Without a fix there is NO size for this branch, and it must not fall back on `jump`:
      // jump <= 1 is the condition for being in here at all, so that fallback could only ever
      // report a sub-degree turn -- and would auto-confirm the very case this exists to make
      // safe, a re-origin with the waypoint abaft the beam whose bearing has not caught up.
      // Leaving the measurement unset means the pending is refused as not sizeable, which is
      // what happened before this branch existed.
      if (geoBrg !== null) {
        const geoTurn = this.angDiffDeg(geoBrg, this.legBrg)
        this.lastTurnKind = 'restart'
        this.lastTurnDeg = Math.max(jump, geoTurn)
        this.lastTurnJumpDeg = jump
        this.lastTurnGeoDeg = geoTurn
        this.lastTurnMovedM = moved
        this.lastTurnXteFromM = Math.round(wasXte * 10) / 10
        this.lastTurnOldHeldMs = now - this.legSince
        this.lastTurnAt = now
        this.legBrg = b
        this.legWp = wp
        this.legSince = now
        this.debug(`re-origin from cross-track collapse ${wasXte.toFixed(1)} -> ${xte.toFixed(1)} m, turn ${this.lastTurnDeg.toFixed(1)}deg`)
        return
      }
      this.debug(`cross-track collapse ${wasXte.toFixed(1)} -> ${xte.toFixed(1)} m but no vessel fix to size it -- manual confirm`)
    }
    ...
    //
    // One observable is deliberately unused, and it is the way out if the geometry ever needs
    // hardening. The branch above assumes the new leg's origin is the old destination, which is
    // exactly what makes it overstate the turn inside a burst of destination changes. The model
    // also carries navigation.courseGreatCircle.previousPoint.position, which would let that
    // assumption be checked rather than trusted. A 2026-08-01 attempt to size legs from
    // previousPoint -> nextPoint instead was reverted for measuring a 90 deg turn as 15: the two
    // paths do not update together, so it paired an old origin with a new destination and got a
    // course neither leg ever had. Read together they could still tell a sequential advance from
    // a burst -- but only with the staleness of BOTH stamps accounted for, which is the part that
    // sank it. Sized from the destinations alone the error at least points at a manual confirm.
    //
  ...
  maybeAutoAdvance () {
    ...
    const xteFromM = this.lastTurnXteFromM
    this.lastTurnXteFromM = null
  ...
  handleIncomingAP (pgn) {
    ...
    if (key in KEY_STATE) {
      ...
      if (st === 'route') {
        ...
        const prevMode = this.commandedMode
        const seq = ++this.cmdSeq
        this.commandMode(st)   // optimistic: drive the firehose display immediately
        this.applyV2(`PUT /state {value:"${st}"}`, (cb) => this.sk.setState(st, cb), (why) => {
          ...
          this.withdrawMode(real || prevMode)
      ...
      if (!this.engaged()) {
        ...
        // The poll, NOT keyMode. The lag cuts both ways here and the two sides are not equal: a
        // Tack refused just after entering wind costs a press, while one let through just after a
        // Wind key reaches a pilot that may still be in auto -- and the V2 provider passes tack
        // through in auto too, so the EV would turn on heading instead of mirroring the wind
        // angle. An unasked-for course change beats a missed keystroke, so this branch keeps the
        // conservative reading. The nudge above has no such asymmetry: it acts either way, only
        // the sign differs, so there the fresher answer is strictly better.
      ...
      if (by.length >= 11 && (by[8] === 0x03 || by[8] === 0x02)) {
        ...
        if (!this.engaged()) {
          ...
          let effSign = sign
          if (this.keyMode() === 'wind') { effSign = -sign }
  ...
  // What the server currently believes about the active leg, read straight off the model for
  // display only. Nothing in the decision path reads this back.
  //
  // Both calculation branches are read, not just the one the leg tracker uses. The plotter picks
  // the branch by a field in 129284, and n2k-signalk maps anything that is not exactly
  // "Great Circle" -- including "unavailable", which is what a Navico MFD sends when it is not
  // navigating -- onto courseRhumbline. So which branch carries the live leg is the plotter's
  // choice, and showing only one of them would leave this blank while the MFD is plainly steering
  // a route.
  //
  // The point of the source field is that Signal K has no other way to answer "who is navigating".
  // The server's Course API tracks it internally as cmdSource but does not expose it, so when the
  // plotter's destination does not appear there is nowhere to look. Two live writers on the same
  // path is the state worth seeing: it means two things are navigating at once, and the model is
  // showing whichever spoke last.
  courseJson (maxAgeMs) {
    const maxAge = maxAgeMs || LEG_MAX_AGE_MS
    const now = Date.now()

    const node = (path) => {
      try { return this.app.getSelfPath(path) || null } catch (e) { return null }
    }
    const ageOf = (n) => {
    ...
    const fresh = (n) => {
      const a = ageOf(n)
      return a !== null && a <= maxAge ? n : null
    }
    // A name the plotter sends as an empty slot: 129285 carries 0 or "" when there is no route,
    // and canboat hands that straight through. Rendering "0" as a route name is worse than blank.
    const name = (n) => {
      if (!n) { return null }
      const v = n.value
    ...
    // A destination has to be an actual fix, not merely a recent write. The plotter publishes
    // 129284 at 1 Hz whether or not it is navigating, and the not-navigating form decodes to
    // {latitude: null, longitude: null} -- fresh, present, and meaningless. Reading that as a live
    // leg would show a destination on the status page whenever the MFD is merely powered on.
    // Null Island is excluded for the same reason it is in nextPointPos: this plotter emits it
    // while re-activating a route.
    const hasFix = (n) => {
      const v = n && n.value
      if (!v || !Number.isFinite(v.latitude) || !Number.isFinite(v.longitude)) { return false }
      return !(v.latitude === 0 && v.longitude === 0)
    }

    // Pick the branch by whichever one has a live destination. Ties go to Great Circle, which is
    // the branch the leg tracker follows.
    let branch = null
    let posNode = null
    for (const b of ['GreatCircle', 'Rhumbline']) {
      const n = fresh(node(`navigation.course${b}.nextPoint.position`))
      if (!n || !hasFix(n)) { continue }
      if (!posNode || ageOf(n) < ageOf(posNode)) { branch = b; posNode = n }
    }
    if (!branch) {
      return { branch: null, routeName: null, nextPointName: null, distanceM: null, bearingDeg: null, source: null, ageMs: null, writers: [], contested: false }
    }

    const p = `navigation.course${branch}.`
    const dist = fresh(node(p + 'nextPoint.distance'))
    const brg = fresh(node(p + 'nextPoint.bearingTrue'))

    // Every source publishing the destination, not just the one that happens to have won. This is
    // the whole point: a second writer is invisible in the merged value.
    const writers = []
    const seen = new Set()
    const add = (src, n) => {
      if (!src || seen.has(src)) { return }
      const a = ageOf(n)
      if (a === null || a > maxAge || !hasFix(n)) { return }
      seen.add(src)
      writers.push({ src, ageMs: Math.round(a) })
    }
    add(posNode.$source, posNode)
    const values = posNode.values || {}
    for (const k of Object.keys(values)) { add(k, values[k]) }
    writers.sort((a, b) => a.ageMs - b.ageMs)

    return {
      branch,
      routeName: name(fresh(node(p + 'activeRoute.name'))),
      nextPointName: name(fresh(node(p + 'nextPoint.name'))),
      distanceM: dist && Number.isFinite(dist.value) ? Math.round(dist.value) : null,
      bearingDeg: brg && Number.isFinite(brg.value) ? Math.round(brg.value * 1800 / Math.PI) / 10 : null,
      source: posNode.$source || null,
      ageMs: Math.round(ageOf(posNode)),
      writers,
      // More than one live publisher of the destination means two navigators are running and the
      // model is showing whichever spoke last.
      contested: writers.length > 1
    }
  }
  ...
  statusJson () {
    ...
    return {
      ...
      // What the server believes about the active leg right now, including who is publishing it.
      // Display only -- see courseJson.
      course: this.courseJson(),
