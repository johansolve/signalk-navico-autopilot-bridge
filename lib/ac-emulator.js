...
const { createPgnStream } = require('./canboat-compat')
...
class ACEmulator {
  ...
  // as the leg-to-leg change. Sampled at 2 Hz (and again at the pending moment). On the jump,
  // record the turn size, how long the OLD leg had been held, and when. On a null bearing
  // (route deactivated / GPS gap) drop the leg state so a later bearing can never be sized
  // against a stale value from a different route.
  sampleTrackBrg () {
    if (this.autoAdvanceMaxDeg <= 0) { return }
    const b = this.selfPathNum('navigation.courseGreatCircle.bearingTrackTrue.value')
    const now = Date.now()
    if (b === null) { this.legBrg = null; this.legSince = 0; this.lastTurnDeg = null; return }
    if (this.legBrg === null) { this.legBrg = b; this.legSince = now; return }
    if (this.angDiffDeg(b, this.legBrg) > 1) {   // bearing jumped -> a waypoint advance
    ...
    if (key in KEY_STATE) {
      ...
      if (by.length >= 11 && (by[8] === 0x03 || by[8] === 0x02)) {
        ...
        if (!this.engaged()) {
          ...
          // water. In the EV's WIND VANE mode the keystroke instead nudges the wind
          // angle by MAGNITUDE: native '+' bears away (|AWA| up). On PORT tack that is
          // a turn to starboard -- exactly what the Vulcan '+' button means -- so leave
          // it. On STARBOARD tack bearing away is a turn to PORT, the opposite of the
          // button, so invert there. Dockside-proven on starboard (native green '+'
          // raised target AWA = bore away = wrong; inverting made it luff/turn stbd).
          // Port stays native (the original "green '+' luffs on port" report is treated
          // as a mis-observation per the skipper). Tack from windAngleSigned() (EV
          // locked datum when fresh, else live AWA); if it is unknown or dead-ahead the
          // tack is ambiguous, so send native rather than guess.
          let effSign = sign
          if (this.state.skApState === 'wind') {
            const awa = this.windAngleSigned()
            if (awa !== null && awa !== 0) {
              if (awa > 0) { effSign = -sign }
            } else {
              this.debug('ChangeCourse in wind but no tack (AWA null/0) -- cannot correct sign, sending native')
            }
          }
          const degN = effSign * Math.round(degMag)
          const delta = (degN + 0.5) * Math.PI / 180
          const flipped = effSign !== sign ? ', stbd-tack-inverted' : ''
  ...
  onPilotMode (msg) {
    ...
    if (this.autoAdvanceMaxDeg > 0 && this.bridge === 'live' &&
