...
class ACEmulator {
  ...
  handleIncomingAP (pgn) {
    ...
    if (key in KEY_STATE) {
      ...
      // side from SK's apparent wind angle (positive to starboard) and delegate
      // to the SK V2 tack endpoint, which the EV-200 turns into a wind-angle
      // mirror: a tack when the wind is forward, a gybe when it is aft. Same
      // command and direction derivation as the legacy large-ChangeCourse branch
      // below. Only meaningful in wind mode, and only when the pilot is engaged
      // and SK state is fresh (never tack a standby or a stale poll).
      if (!this.engaged()) {
        ...
        if (awa === null || awa === 0) {
          this.state.lastMappedEvent = 'Tack/Gybe key but no AWA (ignored)'
          this.debug('Tack/Gybe key 0x11 but no apparent wind angle -- cannot derive direction, ignored')
        } else {
  ...
  statusSummary () {
    ...
    return `${this.acModel} @addr ${addr} | bridge ${this.bridge} | ` +
           ...
           `cmds ${s.ap130850Count} | last ${s.lastMappedEvent || '-'}${wd}`
  }
