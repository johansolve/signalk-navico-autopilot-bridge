...
class ACEmulator {
  ...
  onRawFrame (msg) {
    ...
    try {
      if (!msg || !msg.pgn || !msg.data) { return }
  ...
  // Best MFD name: the device that sends our 130850 if it advertises a product
  // name; otherwise the Navico/B&G chart plotter on the bus (a Navico MFD claims
  // several addresses and the autopilot-controller one often has no product info,
  // while the display/navigator address carries the "Vulcan 7"/"Zeus"/... name).
  bestMfdLabel (sources) {
    ...
    if (this.mfdSrc != null) {
      ...
      const prod = n && ((n.modelVersion && String(n.modelVersion).trim()) || (n.modelId && String(n.modelId).trim()))
      if (prod) {
        return this.deviceLabel({ modelVersion: MFD_RE.test(prod) ? clean(prod) : prod, manufacturerCode: n.manufacturerCode })
      }
      fallback = this.deviceLabel(n)
    }
    // Scan order is the source-address order, which would hit a sub-function address
    // (e.g. the iGPS at src 2) before the display itself, so prefer the Display-class
    // address and keep the first other match only as a stand-in.
    let candidate = null
    for (const bus of Object.keys(sources)) {
      const devs = sources[bus]
      ...
      for (const k of Object.keys(devs)) {
        ...
        if (prod && MFD_RE.test(prod)) {
          const cand = { modelVersion: clean(prod), manufacturerCode: n.manufacturerCode }
          if (n.deviceClass === 'Display') { return this.deviceLabel(cand) }
          if (!candidate) { candidate = cand }
        }
      }
    }
    if (candidate) { return this.deviceLabel(candidate) }
    return fallback
  }
  ...
  findSteeringRole (sources, funcCode, preferMfr) {
    ...
    for (const bus of Object.keys(sources)) {
      ...
      for (const k of Object.keys(devs)) {
        ...
        const prod = (n.modelVersion && String(n.modelVersion).trim()) || (n.modelId && String(n.modelId).trim())
        if (!prod) { continue }
        const label = this.deviceLabel(n)
        if (preferMfr && n.manufacturerCode === preferMfr) { return label }
  ...
  refreshDevices () {
    ...
    this.sk.getSources((err, sources) => {
      ...
      try {
        ...
        const mfd = this.bestMfdLabel(sources)
        if (mfd) { this.mfdName = mfd }
