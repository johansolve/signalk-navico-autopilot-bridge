...
// PROTOCOL-REFERENCE §3.3. TX_PGNS still declares both, as a real AC does.)
//
...
class ACEmulator {
  ...
  sampleTrackBrg () {
    ...
    if (this.angDiffDeg(b, this.legBrg) > 1) {   // bearing jumped -> advance, or a leg re-origin
      ...
      this.lastTurnDeg = this.angDiffDeg(b, this.legBrg)
  ...
  maybeAutoAdvance () {
    ...
    if (kind === 'restart' && this.autoConfirmRestart) {
      ...
      this.applyV2(`restart re-origin ${deg.toFixed(1)}deg (V1 advanceWaypoint)`,
    ...
    if (deg <= this.autoAdvanceMaxDeg) {
      ...
      this.applyV2(`auto-advance ${deg.toFixed(1)}deg (V1 advanceWaypoint)`, (cb) => this.sk.advanceWaypoint(cb))
    } else {
  ...
  confirmNav (via) {
    ...
    this.commandedAt = Date.now()
    if (evFresh && this.evPilotMode === 0x0181) {
      this.state.lastMappedEvent = `Nav confirm (${via}) -> pilot already engaged, latched`
    } else {
      this.state.lastMappedEvent = `Nav confirm (${via}) -> engage track`
  ...
  onPilotMode (msg) {
    ...
    // Resolve the pending window on THIS frame instead of waiting for the 2 Hz poll. The EV can
    // pass through Track-engaged and straight back into pending inside one poll interval -- a
    // Restart pressed right after the confirm does exactly that -- and the poll would then never
    // observe the 0x0181, leaving navPending stuck until its 20 s timeout. navPending blocks the
    // auto-confirm below outright, which is what made such a restart still ask on the P70.
    this.reconcileNavPending()
